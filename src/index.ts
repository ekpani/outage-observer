...
import { fetchStatus, type Level } from "./adapters";
...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Debug: force a poll now
    if (url.pathname === "/debug/poll" && url.searchParams.get("key") === env.DEBUG_KEY) {
    ...
    if (url.pathname === "/debug/alert" && url.searchParams.get("key") === env.DEBUG_KEY) {
